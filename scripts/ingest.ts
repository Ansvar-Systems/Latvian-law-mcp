#!/usr/bin/env tsx
/**
 * Latvian Law MCP -- Ingestion Pipeline
 *
 * Fetches Latvian legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Latvian Parliament) provides free public access to all legislation
 ...
 * License: Latvian legislation is public domain under Art. 4 of the Copyright Act
 */
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Latvian Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseLatvianHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Latvian Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Latvian Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
