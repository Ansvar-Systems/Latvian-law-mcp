/**
 * HTML parser for Latvian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Latvian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Latvian statute.
 *
 ...
 *   </div>
 */
export function parseLatvianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Latvian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Latvian legal text.
 *
 * Latvian definitions typically use patterns like:
...
/**
 * Pre-configured list of key Latvian Acts to ingest.
 *
 * Source: api.sejm.gov.pl (Sejm ELI API)
 * URL pattern: https://api.sejm.gov.pl/eli/acts/DU/{YEAR}/{POZ}/text.html
 *
 * These are the most important Latvian statutes for cybersecurity, data protection,
