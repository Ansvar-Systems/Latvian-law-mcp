#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Latvian Law MCP server.
 *
...
const DEFAULT_DB_PATH = path.resolve(__dirname, '../../../Latvian-law-mcp/data/database.db');
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'at_gov_lv',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  committee TEXT,
  proposer TEXT,
  source TEXT NOT NULL DEFAULT 'saeima',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Latvian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'LV');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'at.gov.lv,satv.tiesa.gov.lv');
    upsertMeta.run('source_prep_works', 'saeima.lv');
    upsertMeta.run('source_agency', 'dvi.gov.lv,kp.gov.lv');
  });
  updateMeta();
