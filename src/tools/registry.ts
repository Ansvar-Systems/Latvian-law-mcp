/**
 * Tool registry for Latvian Law MCP Server.
...
import { getLatvianImplementations, type GetLatvianImplementationsInput } from './get-latvian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Likumi.lv (Latvian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Latvian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Latvian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Latvian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Latvian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Latvian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Latvia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Latvian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Latvian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Latvian statute references or aligns with. ' +
      'As an EU Member State, Latvia transposes EU directives and implements EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Latvian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_latvian_implementations',
    description:
      'Find all Latvian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Latvian statutes. ' +
      'Note: Latvia is an EU Member State and transposes EU directives into national law.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Latvian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_latvian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Latvian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Latvian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Latvian statute or provision. ' +
      ...
      'Note: As an EU Member State, Latvia is bound by EU law. This checks transposition and compliance status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Latvian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_latvian_implementations':
          result = await getLatvianImplementations(db, args as unknown as GetLatvianImplementationsInput);
          break;
